...
function normalizarMapaExperiencia(textoUsuario) {
  ...
  try {
    // Retraso defensivo para evitar el Error 429: Cuota de peticiones Gemini excedida
    ...
    const response = fetchConReintentos(url, options);
    ...
    if (resJson.candidates && resJson.candidates[0].content) {
      ...
      if (inicio !== -1 && fin !== -1) {
        return JSON.parse(textoIA.substring(inicio, fin));
      }
    }
    return null; 
  } catch (e) {
...
Devuelve ÚNICAMENTE un objeto JSON con esta estructura EXACTA (todo en minúsculas):
{
  ...
  try {
    ...
    const response = fetchConReintentos(url, options);
    const resText = response.getContentText();
    console.log("DEBUG MEGA-CALL 1 (Diagramas): " + resText);
    const resJson = JSON.parse(resText);
...
Devuelve ÚNICAMENTE un objeto JSON con esta estructura EXACTA:
{
  ...
  try {
    ...
    const response = fetchConReintentos(url, options);
    const resText = response.getContentText();
    console.log("DEBUG MEGA-CALL 2 (Tablas): " + resText);
    const resJson = JSON.parse(resText);
